...
export PATH
# wget http://7xs2gz.dl1.z0.glb.clouddn.com/openvpn.sh && bash openvpn.sh 2>&1 | tee openvpn.log
clear;
# Logo 	******************************************************************
CopyrightLogo='
==========================================================================
...
rpm -ivh http://7xs2gz.dl1.z0.glb.clouddn.com/64-epel-release-5-4.noarch.rpm
else
rpm -ivh http://7xs2gz.dl1.z0.glb.clouddn.com/32-epel-release-5-4.noarch.rpm
fi
fi

if [ $version == "6" ];then
if [ $(getconf LONG_BIT) = '64' ] ; then
rpm -ivh http://7xs2gz.dl1.z0.glb.clouddn.com/64-epel-release-6-8.noarch.rpm
else
rpm -ivh http://7xs2gz.dl1.z0.glb.clouddn.com/32-epel-release-6-8.noarch.rpm
fi
fi

if [ $version == "7" ];then
rpm -ivh http://7xs2gz.dl1.z0.glb.clouddn.com/epel-release-latest-7.noarch.rpm
fi
...
cd /etc/
rm -rf ./sysctl.conf
wget http://7xs2gz.dl1.z0.glb.clouddn.com/sysctl.conf
sleep 3
...
cd /etc/openvpn/
rm -rf ./server.conf
wget http://7xs2gz.dl1.z0.glb.clouddn.com/server.conf
chmod 0755 ./server.conf
cd /etc/squid/
rm -f ./squid.conf
wget http://7xs2gz.dl1.z0.glb.clouddn.com/squid.conf
chmod 0755 /etc/squid/squid.conf
sleep 2
squid -z  
squid -s   
cd /etc/openvpn/
wget http://7xs2gz.dl1.z0.glb.clouddn.com/EasyRSA-2.2.2.tar.gz
...
echo "写入客户端证书"
sleep 2
cat ovpn.11 user01.crt>ovpn.12
echo '</cert>
<key>' >ovpn.13
cat ovpn.12 ovpn.13>ovpn.14
echo "写入客户端密钥"
sleep 2
cat ovpn.14 user01.key>ovpn.15
echo "写入后端代码"
echo '</key>
ns-cert-type server
comp-lzo
verb 3
' >ovpn.16
echo "生成User01.ovpn文件"
cat ovpn.15 ovpn.16 >User01.ovpn
echo "配置文件制作完毕"
echo
sleep 3
clear
tar -zcvf openvpn.tar.gz ./{User01.ovpn,ca.crt,user01.{crt,key}}
rm -rf ./{myip,ovpn.1,ovpn.2,ovpn.3,ovpn.4,ovpn.5,ovpn.6,ovpn.7,ovpn.8,ovpn.9,ovpn.10,ovpn.11,ovpn.12,ovpn.13,ovpn.14,ovpn.15,ovpn.16,User01.ovpn,ca.crt,user01.{crt,key}}
clear
# OpenVPN Installing ****************************************************************************
echo 
echo "正在创建下载链接："
...
echo '=========================================================================='
echo 您的IP是：$myip 
echo （检测结果与您实际IP不符合/空白，请自行修改User01.ovpn配置）
Client='
                             OpenVPN-2.3.10 安装完毕                                
                         Powered by sbwml.cn 2015-2016                     
                              All Rights Reserved                  
